...
export class ServiceReportComponent implements OnInit {
  ...
  GeneratePDF(preview: boolean) {
    if (!preview) {
      if (confirm(`Do you really want to send the service report to below customer emails: ${this.ServiceRequest.operatoremail} and ${this.ServiceRequest.email}`)) {
        this.pdf(preview)
      }
    } else {
      this.pdf(preview)
    ...
    this.ServiceReportService.getView(this.ServiceReportId)
      ...
      .subscribe({
        next: (data: any) => {
          ...
          preview ? pdfMake.createPdf(docDefinition).open() :
