...
import {AccountService, ContactService, NotificationService, ProfileService, ServiceReportService} from '../_services';
...
export class ServiceReportListComponent implements OnInit {
  ...
  pdf(ServiceReportId) {
    this.ServiceReportService.getView(ServiceReportId)
      .pipe(first())
      .subscribe({
          next: (data: any) => {
            console.log(data);
            ...
            this.preventivemaintenancesService.getById(ServiceReportId)
              .pipe(first())
              .subscribe({
                next: (prev: any) => {
                  {
